...
const PrimaryButton: React.FC<PrimaryButtonProps> = ({ text, hoverClass, gsapClass = '', className = '', ...props }) => {
    return (
        <div className={`${gsapClass} flex`}>
            <button
                className={`overflow-hidden group rounded relative z-[1] transition_slow text-sm bg-deep-blue px-[16px] py-[13px] ${className}`}
                {...props}
            >
                <span className="group-hover:drop-shadow-3xl transition_slow font-inter font-semibold text-nowrap sm:text-base text-white">{text}</span>
